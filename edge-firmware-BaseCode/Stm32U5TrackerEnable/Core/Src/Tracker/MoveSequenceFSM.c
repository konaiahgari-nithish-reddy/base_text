...
SmartTrakOrientation acquisitionAngle;	// Saurabh
...
void MoveSequenceFSM(void)
{
	...
	case ST_MOVE_SEQ_STOPPED:				// all motion stopped
	{
		...
		if((IsCommandComplete(MOTOR_AZIMUTH) == FALSE) || (IsCommandComplete(MOTOR_ELEVATION) == FALSE))			// check for command complete
		{
		...
		if (IS_BITSET(efMoveSequenceEvents, EF_MOVE_SEQ_FIND_END_POINTS))
		{
			...
			DisplayMessage("Initial Mechanical Orientation\r\n", WAIT_FOR_DISPLAY);
...
#ifdef NOT_USED
			if (bFindEndPointsHasRun == FALSE)
			{
				DisplayMessage("Please Run FindEndPoints before SPA Tracking\r\n", WAIT_FOR_DISPLAY);
			}
			else
#endif
			{
		...
		if ((IS_BITSET(efPanelPositionEvents, EF_PANEL_POS_WIND_STOW))
				||(IS_BITSET(efPanelPositionEvents, EF_PANEL_POS_END_WIND_STOW)))
		{
	...
	case ST_MOVE_SEQ_ACTIVE:								// processing an event sequence
		...
		case MOVE_SEQ_STEP_WAIT_TIMER1:					// wait for timer 1 to decrement to 0
			...
			if (fgwTimer1 == (WORD)0)					// timer finished?
			{
				++fgptrMoveSequence;					// bump step pointer
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("\r\nTimer 1 Done\r\n", NO_WAIT_FOR_DISPLAY);
			...
			else
			{
				...
				DisplayStrSequence("Wait for Timer 1 ");
#endif
			}
			break;
		...
		case MOVE_SEQ_STEP_WAIT_TIMER2:					// wait for timer 1 to decrement to 0
			// timers are bumped (decremented) during MoveSequenceFSM_Tick()
			if (fgwTimer2 == (WORD)0)					// timer finished?
			{
				++fgptrMoveSequence;					// bump step pointer
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("\r\nTimer 2 Done\r\n", NO_WAIT_FOR_DISPLAY);
			...
			else
			{
				// timer has not yet finished
#ifdef USE_MOVE_SEQ_FSM_TIMER_WAIT_VERBOSE
				DisplayStrSequence("Wait for Timer 2 ");
#endif
			}
			break;
		...
		case MOVE_SEQ_STEP_WAIT_TIMER3:					// wait for timer 1 to decrement to 0
			...
			else
			{
				// timer has not yet finished
#ifdef USE_MOVE_SEQ_FSM_TIMER_WAIT_VERBOSE
				DisplayStrSequence("Wait for Timer 3 ");
#endif
			}
			break;
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("Start Counter 1\r\n", NO_WAIT_FOR_DISPLAY);
#endif
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("Start Counter 2\r\n", NO_WAIT_FOR_DISPLAY);
#endif
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("Start Counter 3\r\n", NO_WAIT_FOR_DISPLAY);
#endif
		...
		case MOVE_SEQ_STEP_SKIP_ON_COUNTER1:			// decrement and skip next step on counter 1 = 0
			...
			if (fgwCounter1 == (WORD)0)					// counter finished?
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("Skip on Counter 1 == 0\r\n", NO_WAIT_FOR_DISPLAY);
#endif
		...
		case MOVE_SEQ_STEP_SKIP_ON_COUNTER2:			// decrement and skip next step on counter 2 = 0
			...
			if (fgwCounter2 == (WORD)0)					// counter finished?
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("Skip on Counter 2 == 0\r\n", NO_WAIT_FOR_DISPLAY);
#endif
				++fgptrMoveSequence;					// bump step pointer to skip next step
			}
			break;
		...
		case MOVE_SEQ_STEP_SKIP_ON_COUNTER3:			// decrement and skip next step on counter 3 = 0
			...
			if (fgwCounter3 == (WORD)0)					// counter finished?
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("Skip on Counter 3 == 0\r\n", NO_WAIT_FOR_DISPLAY);
#endif
				++fgptrMoveSequence;					// bump step pointer to skip next step
			}
			break;
		...
		case MOVE_SEQ_STEP_EXEC_SPA_COMMAND:			// execute command by setting an event flag
			...
			if (ReadInclinometerSample(&pgInclination) == FALSE)
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("FAILED Inclinometer\r\n", WAIT_FOR_DISPLAY);
#endif
...
//			}
...
#ifdef USE_AZIMUTH
			if (fgptrMoveSequence->eAxis == AXIS_AZIMUTH)
			{
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
					DisplayMessage("Short Azimuth Move, No Need to Execute Command\r\n", WAIT_FOR_DISPLAY);
#endif
					++fgptrMoveSequence;					// bump step pointer
				...
				if (fgptrMoveSequence->eCommand == EF_MTN_CMD_RUN)			// this is typical of SPA tracking
				{
					// NOTE: we do not process PWM_DIR_STOPPED here, nothing to do!
					if (fgSPAMove.eAzimuthDirection == PWM_DIR_FORWARD)
					{
						// Run forward
						bTrackerDirection = 2;
						BITSET(efMotionPhaseCommands[fgptrMoveSequence->eAxis], EF_MTN_CMD_RUN_FWD);
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
						DisplayMessage("Start Cmd Timer\r\n", NO_WAIT_FOR_DISPLAY);
#endif
					...
					else if (fgSPAMove.eAzimuthDirection == PWM_DIR_REVERSE)
					{
						// Run reverse
						bTrackerDirection = 1;
					...
					else if (fgSPAMove.eAzimuthDirection == PWM_DIR_STOPPED)
					{
						// Stopped (huh?)
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
						DisplayMessage("Azimuth Move STOPPED\r\n", NO_WAIT_FOR_DISPLAY);
#endif
					}
				}
				else
			...
			}		//  (fgptrMoveSequence->eAxis == AXIS_AZIMUTH)
#endif	// USE_AZIMUTH
...
#ifdef USE_ELEVATION
				if (fgptrMoveSequence->eAxis == AXIS_ELEVATION)
				{
					...
					if (ABS(fgSPAMove.lElevationMoveTicks) < RUN_MINIMUM_MOVE_TICKS(fgptrMoveSequence->eAxis))
					{
						...
						DisplayMessage("Short Elevation Move, No Need to Execute Command\r\n", WAIT_FOR_DISPLAY);
#endif
						++fgptrMoveSequence;					// bump step pointer
						break;
					}
					...
					{
						// NOTE: we do not process PWM_DIR_STOPPED here, nothing to do!
						if (fgSPAMove.eElevationDirection == PWM_DIR_FORWARD)
						{
							// Run forward
							BITSET(efMotionPhaseCommands[fgptrMoveSequence->eAxis], EF_MTN_CMD_RUN_FWD);
#ifdef USE_MOVE_SEQ_FSM_STEP_VERBOSE
						...
						else if (fgSPAMove.eElevationDirection == PWM_DIR_REVERSE)
						{
							// Run reverse
							BITSET(efMotionPhaseCommands[fgptrMoveSequence->eAxis], EF_MTN_CMD_RUN_REV);
#ifdef USE_MOVE_SEQ_FSM_STEP_VERBOSE
						...
						else if (fgSPAMove.eElevationDirection == PWM_DIR_STOPPED)
						{
							// Stopped (huh?)
							BITCLEAR(efMotionPhaseCommands[fgptrMoveSequence->eAxis], EF_MTN_CMD_RUN_FWD);
							BITCLEAR(efMotionPhaseCommands[fgptrMoveSequence->eAxis], EF_MTN_CMD_RUN_REV);
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
							DisplayMessage("Elevation Move STOPPED\r\n", NO_WAIT_FOR_DISPLAY);
#endif
						}
					}
					else
				...
				}		//  (fgptrMoveSequence->eAxis == AXIS_ELEVATION)
#endif	// USE_ELEVATION
			...
			++fgptrMoveSequence;						// bump step pointer
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("SPA Execute Command\r\n", NO_WAIT_FOR_DISPLAY);
#endif
		...
		case MOVE_SEQ_STEP_WAIT_SPA_COMMAND:				// wait for command complete
			if ((eMoveState == PANEL_POSITION_HOLD) AND (eTrackState == PANEL_POSITION_HOLD))
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("No Need to wait for Command\r\n", WAIT_FOR_DISPLAY);
#endif
				++fgptrMoveSequence;					// bump step pointer
				break;
			}
			...
			if (IsCommandComplete(fgptrMoveSequence->eAxis) IS_TRUE)			// check for command complete
			{
				...
				DisplayStrWait();			// start display (serial output) of line
#endif
				...
				{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
					DisplayMessage("Failed Read RTCC\r\n", WAIT_FOR_DISPLAY);
#endif
			...
			else
			{
				...
				// check for timeout
				// timers are bumped (decremented) during MoveSequenceFSM_Tick()
				if (fgwCmdTimer == (WORD)0)					// timer finished?
				{
					...
					AddDisplayStr("MoveSequence.c: Line 1621\n");
					DisplayStr();
				...
				else
				{
					...
					DisplayStrSequence(szfnDisplayStr);
#endif
				}
		...
		case MOVE_SEQ_STEP_EXEC_COMMAND:				// execute command by setting an event flag
			// start command
			if ((fgptrMoveSequence->eAxis == AXIS_AZIMUTH) || (fgptrMoveSequence->eAxis == AXIS_ELEVATION))
			{
				SetCommandStarted(fgptrMoveSequence->eAxis);// mark command as started so we cannot misinterpret completion
				...
				++fgptrMoveSequence;						// bump step pointer
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("Execute Command\r\n", NO_WAIT_FOR_DISPLAY);
#endif
			}
			else
			{
...
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("Restore Previously Stored SPAOrientation\r\n", WAIT_FOR_DISPLAY);
#endif
		...
		case MOVE_SEQ_STEP_CALC_SUN_POSITION:				// save current sun position to fgSPAOrientation
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
			DisplayMessage("Calculate Sun Position to SPAOrientation\r\n", WAIT_FOR_DISPLAY);
#endif
			...
			acquisitionAngle = fgSPAOrientation; // Saurabh
		...
		case MOVE_SEQ_STEP_SET_SPA_MOVE_DISTANCE:			// set distance to move
			...
			else if (fgptrMoveSequence->eAxis == AXIS_ELEVATION)
			{
		...
		case MOVE_SEQ_STEP_SPA_SKIP_ON_MOVE_DONE:
			...
			DisplayStrWait();			// start display (serial output) of line
#endif
			if (eMoveState == PANEL_POSITION_HOLD)
			{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
				DisplayMessage("PANEL_POSITION_HOLD, Skip on Move Complete\r\n", WAIT_FOR_DISPLAY);
#endif
				++fgptrMoveSequence;					// bump step pointer
				++fgptrMoveSequence;					// bump step pointer
				break;
			...
			if (fgptrMoveSequence->eAxis == AXIS_AZIMUTH)
			{
				...
				case PWM_DIR_FORWARD:
					...
					if ((fgCurrentLocalOrientation.fAzimuth > (fgOrientation1.fAzimuth + (pgfMoveDistanceDegrees[MOTOR_AZIMUTH] * MOVE_INCREMENT_COUNT)))
							||		// OR an additional half of an incremental move will put us past the desired movement, move is complete
					...
					{
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
						DisplayMessage("Skip on Move Complete\r\n", NO_WAIT_FOR_DISPLAY);
#endif
						++fgptrMoveSequence;					// bump step pointer to skip next step
					}
					break;
				...
				default:
#if defined(USE_MOVE_SEQ_FSM_STEP_VERBOSE)
					DisplayMessage("Skip on Move Unknown Direction\n", NO_WAIT_FOR_DISPLAY);
#endif
					break;
				}
			}
