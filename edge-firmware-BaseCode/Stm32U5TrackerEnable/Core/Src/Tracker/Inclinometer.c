...
float convert_to_g(int16_t raw_value) {
	return raw_value * 0.061f / 1000.0f; // Convert mg to g
//	return raw_value * 0.244f; // Convert mg to g
}
...
BOOL ReadInclinometerSample(PTR_INCLINOMETER_SAMPLE Inclination)
{
	...
	if (gIsMma845Enabled)
	{
		...
		if (ReadInclinometerArray(OUT_X_MSB_REG, &(Inclination->Acc_Sample.RAW_Data[0]), ACCELEROMETER_SAMPLE_LEN) IS_TRUE)
		{
			...
			if ((fResult > 0.01) || (fResult < -0.01))			// check for divide by 0
			{
		...
		if (Read_WhoAmI_Register() IS_FALSE)				// enable analog blocks for measurement
		{
			RuntimeError(INCLINOMETER_ERROR_ACCESS_FAILURE);
			return FALSE;							// communications error
		}

		if (Enable_XYZ_Axis() IS_FALSE)				// enable analog blocks for measurement
		{
			RuntimeError(INCLINOMETER_ERROR_ACCESS_FAILURE);
			return FALSE;							// communications error
		}
		...
		do
		{
			if (ReadInclinometerRegister(LIS2HH12_STATUS, &bStatusRegister) IS_FALSE)
			{
				RuntimeError(INCLINOMETER_ERROR_ACCESS_FAILURE);
				return FALSE;
			}
		...
		// read data from Accelerometer
		if (ReadInclinometerArray(LIS2HH12_OUT_X_L, &(Inclination->Acc_Sample.RAW_Data[0]), ACCELEROMETER_SAMPLE_LEN) IS_TRUE)
		{
//			Inclination->nAcc_X_Value = (INT16)Inclination->Acc_Sample.RAW_Data[0];
//			sResult = Inclination->nAcc_X_Value;
//			fResult = convert_to_g(sResult);
//			Inclination->fAcc_X_Value = fResult;
			//*****************************
			//			X Axis
			//*****************************
			...
			sResult = (INT16)(Inclination->Acc_Sample.RAW_Data[1] * 256);					// shift 8MSB to correct position in INT16 fResult
			// check for negative value
			if ((Inclination->Acc_Sample.RAW_Data[1] & OUT_SIGN_MASK) IS OUT_SIGN_MASK)	// check for negative number
			{
				// value is negative, so keep track of negative state
				bNegative = TRUE;
			}
			// read 4LSB of x axis; data is in the MSB position
			sResult += (INT16)(Inclination->Acc_Sample.RAW_Data[0]);							// shift 8LSB to LSB position
			if (bNegative IS TRUE)							// check for 8MSB negative number
			{
				sResult |= (INT16)VALUE_SIGN_BIT;			// restore sign bit in MSB location, also sets 3 unused bits
			}
			...
			//			Y Axis
			//*****************************
//			Inclination->nAcc_Y_Value = (INT16)Inclination->Acc_Sample.RAW_Data[2];
//			sResult = Inclination->nAcc_Y_Value;
//			fResult = convert_to_g(sResult);
//			Inclination->fAcc_Y_Value = fResult;
			bNegative = FALSE;
			sResult = (INT16)(Inclination->Acc_Sample.RAW_Data[3] * 256);				// shift 8MSB to correct position in INT16 result
			// check for negative value
			if ((Inclination->Acc_Sample.RAW_Data[3] & OUT_SIGN_MASK) IS OUT_SIGN_MASK)	// check for negative number
			{
				// value is negative, so keep track of negative state
				bNegative = TRUE;
			}
			// read 4LSB of x axis; data is in the MSB position
			sResult += (Inclination->Acc_Sample.RAW_Data[2]);						// shift 4LSB to LSB position
			if (bNegative IS TRUE)							// check for 8MSB negative number
			{
				sResult |= (INT16)VALUE_SIGN_BIT;			// restore sign bit in MSB location, also sets 3 unused bits
			}
			...
			//			Z Axis
			//*****************************
//			Inclination->nAcc_Z_Value = (INT16)Inclination->Acc_Sample.RAW_Data[4];
//			sResult = Inclination->nAcc_Z_Value;
//			fResult = convert_to_g(sResult);
//			Inclination->fAcc_Z_Value = fResult;
			bNegative = FALSE;
			sResult = (INT16)(Inclination->Acc_Sample.RAW_Data[5] * 256);				// shift 8MSB to correct position in INT16 result
			// check for negative value
			if ((Inclination->Acc_Sample.RAW_Data[5] & OUT_SIGN_MASK) IS OUT_SIGN_MASK)	// check for negative number
			{
				// value is negative, so keep track of negative state
				bNegative = TRUE;
			}
			// read 4LSB of x axis; data is in the MSB position
			sResult += (Inclination->Acc_Sample.RAW_Data[4]);						// shift 4LSB to LSB position
			if (bNegative IS TRUE)							// check for 8MSB negative number
			{
				sResult |= (INT16)VALUE_SIGN_BIT;			// restore sign bit in MSB location, also sets 3 unused bits
			}
...
//			Inclination->fX_Angle = atan2f(Inclination->fAcc_Y_Value, Inclination->fAcc_Z_Value);
//			Inclination->fY_Angle = atan2f(-(Inclination->fAcc_X_Value), sqrt(Inclination->fAcc_Y_Value * Inclination->fAcc_Y_Value + Inclination->fAcc_Z_Value * Inclination->fAcc_Z_Value));
 		}
		else
		{
			RuntimeError(INCLINOMETER_ERROR_ACCESS_FAILURE);
...
// return X, Y, Z values and X, Y 3D Angles in a formatted string
	...
	void MotionSensor_Tick(enum tagMotors eMotor)
	{
		...
		if ((GetMotionState(eMotor) IS ST_MOTION_CONSTANT_SPEED) || (GetMotionState(eMotor) IS ST_MOTION_CON_INCREASE_PWM) || (GetMotionState(eMotor) IS ST_MOTION_CON_DECREASE_PWM))
		{
		...
		case PHASE_ACCELERATION:			// accelerating speed
			...
			case PHASE_COASTING:				// motor is OFF, some coasting MAY occur
				if ((pgeMotionType[eMotor] IS MOTION_COASTING) || (pgeMotionType[eMotor] IS MOTION_BRAKING))
				{
					;
				}
				else
