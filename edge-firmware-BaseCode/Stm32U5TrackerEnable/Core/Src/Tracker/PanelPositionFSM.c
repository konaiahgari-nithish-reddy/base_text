...
enum tagSunPositionState eTempState = ST_PANEL_POS_POWER_UP;
...
enum tagPanelPositionMovement PanelPositionFSM(SmartTrakOrientation *ptrOrientation)
{
	...
	case ST_PANEL_POS_INIT:
		...
		if ((ptrOrientation->fAzimuth < (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
				|| (ptrOrientation->fAzimuth > (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward - ptrRAM_SystemParameters->fAZ_Offset))
#ifdef USE_ELEVATION
	...
	case ST_PANEL_POS_HOLD:
		...
		/*			if ( (ptrOrientation->fAzimuth > (ptrRAM_SystemParameters->fAZ_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
				...
				&& (ptrOrientation->fElevation > (ptrRAM_SystemParameters->fEL_SoftLimit_Reverse - ptrRAM_SystemParameters->fEL_Offset))
				&& (ptrOrientation->fElevation < (ptrRAM_SystemParameters->fEL_SoftLimit_Forward - ptrRAM_SystemParameters->fEL_Offset))
				&& (ptrOrientation->fElevation <= ptrRAM_SystemParameters->fEL_NightStowThreshold) )
				{
				eSunPosState = ST_PANEL_POS_NIGHT_STOW;
				fSubStateStatus = SUBSTATE_NOT_DONE;
				bSunPosFSMSequenceCtr = 0;
	...
	case ST_PANEL_POS_NIGHT_STOW:
		...
		// check for SPA Orientation within Soft Limits && elevation above Night Stow threshold
		if ( (ptrOrientation->fAzimuth >= (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
				&& (ptrOrientation->fAzimuth <= (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward - ptrRAM_SystemParameters->fAZ_Offset))
#ifdef USE_ELEVATION
				&& (ptrOrientation->fElevation > (ptrRAM_SystemParameters->fEL_SoftLimit_Reverse - ptrRAM_SystemParameters->fEL_Offset))
	...
	case ST_PANEL_POS_POWER_UP:			// initial state, only at power up
		...
		eTempState = ST_PANEL_POS_POWER_UP;
		break;


	case ST_PANEL_POS_INIT:
		// intialize the current position (this is THE point of initialization during startup
#if defined(USE_SINGLE_POLAR_AXIS) && defined(USE_MMA8452Q_INCLINOMETER)
		...
		eTempState = ST_PANEL_POS_INIT;
		break;

	case ST_PANEL_POS_TRACKING:
		// no need to change orientation
		// ==> put code here to limit Orientation Azimuth to the 'Single Axis' soft limits, without considering the offset values. Seems redundant.
		eRetVal = PANEL_POSITION_MOVE;
		if(ptrOrientation->fBackTrack IS_NOT 0)
			eTempState = ST_PANEL_POS_BT;
		else
			eTempState = ST_PANEL_POS_TRACKING;
		break;
	...
	case ST_PANEL_POS_HOLD:
		...
		if (ptrOrientation->fAzimuth <= (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
		{
			ptrOrientation->fAzimuth = (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset);
		}
		else if (ptrOrientation->fAzimuth >= (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward - ptrRAM_SystemParameters->fAZ_Offset))
		{
			ptrOrientation->fAzimuth = (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward - ptrRAM_SystemParameters->fAZ_Offset);
		}
		...
		case ST_PANEL_POS_NIGHT_STOW:
			...
			eTempState = ST_PANEL_POS_NIGHT_STOW;
			switch(bSunPosFSMSequenceCtr)
			{
			case 0:
				// first entry into state
			...
			case ST_PANEL_POS_WIND_STOW:
				...
				eTempState = ST_PANEL_POS_WIND_STOW;
				switch(bSunPosFSMSequenceCtr)
				{
				case 0:
					// first entry into state
