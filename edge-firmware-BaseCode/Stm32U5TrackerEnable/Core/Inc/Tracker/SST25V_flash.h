...
void SST25_write_status(uint8_t);
void flash_SPI_write(uint8_t*, uint16_t);
void flash_SPI_read(uint8_t *, uint16_t);
uint8_t flash_SPI_read_write_byte(uint8_t);
...
void SST25_sector_erase(uint32_t, uint8_t);
