...
// menu functions
BOOL DisplayMenu(UART_MODULE UARTid);
BOOL DisplayStatus(UART_MODULE UARTid);
BOOL DisplayResult(UART_MODULE UARTid);

int MainMenu(char);
int UtilityMenu(char);
int StatusMenu(char);
int MotionSettingsMenu(UART_MODULE UARTid, char);
int OpenLoopMoveMenu(UART_MODULE UARTid, char);
int ClosedLoopMoveMenu(UART_MODULE UARTid, char);
int MoveSequenceMenu(UART_MODULE UARTid, char cKeystroke);
int SystemParametersMenu(UART_MODULE UARTid, char);
int LocationParametersMenu(UART_MODULE UARTid, char);
int AzimuthParametersMenu(UART_MODULE UARTid, char);
int ElevationParametersMenu(UART_MODULE UARTid, char);
int BacktrackingParametersMenu(UART_MODULE UARTid, char);
int SetRTCCDateTimeMenu(UART_MODULE UARTid, char);
int ReadInputsMenu(UART_MODULE UARTid, char);
int TestMenu(UART_MODULE UARTid, char);
...
PRIVATE_INIT enum tagMenus eCurrentMenu = MAIN_MENU;
...
FILE_GLOBAL	const char **fgpMenuText;
...
//	"\x1B[2J\x1B[0;0H==SmartTrak Solar Panel Controller: (" __DATE__ "," __TIME__ ")",
    //"\n\rR2",
    "=R3=SSPC= (" __DATE__ "," __TIME__ "):Version-"Version_SAT"",
	#ifdef USE_ELEVATION
	"1) * Run UP (fwd) to end",
	#endif
...
{
    static unsigned char buff[50],f;
	...
	switch(stMenuState)
		...
		case ST_MENU_DISPLAY_MENU:
        	...
        	if ((fSubStateStatus IS SUBSTATE_DONE) AND (eMenuLine IS MENU_LINE))
				{
        	...
        	else if ((fSubStateStatus IS SUBSTATE_DONE) AND (eMenuLine IS MENU_LAST_LINE))
				{
...
//			break;
		...
		case ST_MENU_DISPLAY_STREAMING_DATA:
			...
			if ((fSubStateStatus IS SUBSTATE_NOT_DONE) AND (bMenuFSMSequenceCtr IS 0))
				{
				eStreamLine = STREAM_FIRST_LINE;				// this is not actually used anywhere..
				}
			// check for display of ONE LINE of streaming data complete
        	else if ((fSubStateStatus IS SUBSTATE_DONE) AND (eStreamLine IS STREAM_LINE))
				{
			...
			// check for done with all available data
        	else if ((fSubStateStatus IS SUBSTATE_DONE) AND (eStreamLine IS STREAM_LAST_LINE))
				{
                // sequence is complete, so bump state
				fSubStateStatus = SUBSTATE_NOT_DONE;
				bMenuFSMSequenceCtr = 0;
		...
		case ST_MENU_DISPLAY_RESULT:
        	...
        	if ((fSubStateStatus IS SUBSTATE_DONE) AND (eResultLine IS RESULT_LINE))
				{
				// we are done with displaying ONE line, but we are not done with ALL lines
			...
			// check for done with all available data
        	else if ((fSubStateStatus IS SUBSTATE_DONE) AND ((eResultLine IS RESULT_LAST_LINE) OR (eResultLine IS RESULT_SINGLE_LINE)) )
				{
                // we are done with last line and sequence is complete, so bump state
				fSubStateStatus = SUBSTATE_NOT_DONE;
				bMenuFSMSequenceCtr = 0;
		...
		case ST_MENU_DISPLAY_MENU:
			switch(bMenuFSMSequenceCtr)
				...
				case 1:
					...
					if (DisplayMenu(UARTid) IS_FALSE)				// returns FALSE if nothing MORE to display
						{
				...
				case 2:
					...
					if (IsTransmitComplete(UARTid) IS_TRUE)
						{
						++bMenuFSMSequenceCtr;						// force next substate on next FSM entry, actually an invalid state
		...
		case ST_MENU_WAIT_FOR_KEYSTROKE:
			switch(bMenuFSMSequenceCtr)
				...
				case 0:
					...
					// if there is already a byte available, skip UART_StartReceive() so we do not lose it..
					if (AnyRxDataAvailable(UARTid) IS_FALSE)
						{
						IGNORE_RETURN_VALUE StartRx(UARTid);
						}
					++bMenuFSMSequenceCtr;			// force next substate on next FSM entry
					break;
				...
				case 1:
					...
					if (AnyRxDataAvailable(UARTid) IS_TRUE)
						{
						pgcInputBuffer[0] = (char)tolower((int)ReadRxdData(UARTid));
						++bMenuFSMSequenceCtr;		// force next substate on next FSM entry
						fSubStateStatus = SUBSTATE_DONE;
						}
                    
//                    while (AnyRxDataAvailable(UARTid) IS_TRUE)
//						{
//						buff[f++] = (char)tolower((int)ReadRxdData(UARTid));
////						++bMenuFSMSequenceCtr;		// force next substate on next FSM entry
////						fSubStateStatus = SUBSTATE_DONE;
//						}
//  ++bMenuFSMSequenceCtr;
...
//                       	++bMenuFSMSequenceCtr;		// force next substate on next FSM entry
//                        fSubStateStatus = SUBSTATE_DONE;
//                        pgcInputBuffer[0]=buff[15];
//                        memset(buff,0,sizeof(buff));
//                        f=0;
//                    }

				#ifdef DOES_NOT_WORK_CORRECTLY	// kludge that does not work correctly
					else if ((eCurrentMenu IS MOVE_SEQUENCE_MENU) AND (IsMoveSequenceComplete() IS_FALSE))
						{
		...
		case ST_MENU_PROCESS_COMMAND:
			switch(eCurrentMenu)
				...
				case MOTION_SETTINGS_MENU:					// may be single pass or multiple pass display
					fSelection = MotionSettingsMenu(UARTid, pgcInputBuffer[0]);
					break;

				case OPEN_LOOP_MOVE_MENU:								// always single pass display
					fSelection = OpenLoopMoveMenu(UARTid, pgcInputBuffer[0]);
					break;

				case CLOSED_LOOP_MOVE_MENU:								// always single pass display
					fSelection = ClosedLoopMoveMenu(UARTid, pgcInputBuffer[0]);
					break;

				case MOVE_SEQUENCE_MENU:					// always single pass display
					fSelection = MoveSequenceMenu(UARTid, pgcInputBuffer[0]);
					break;

				case SYSTEM_PARAMETERS_MENU:
					fSelection = SystemParametersMenu(UARTid, pgcInputBuffer[0]);
					break;

				case LOCATION_PARAMETERS_MENU:
					fSelection = LocationParametersMenu(UARTid, pgcInputBuffer[0]);
					break;

				case AZIMUTH_PARAMETERS_MENU:
					fSelection = AzimuthParametersMenu(UARTid, pgcInputBuffer[0]);
					break;

				case ELEVATION_PARAMETERS_MENU:
					fSelection = ElevationParametersMenu(UARTid, pgcInputBuffer[0]);
					break;

				case BACKTRACKING_PARAMETERS_MENU:
					fSelection = BacktrackingParametersMenu(UARTid, pgcInputBuffer[0]);
					break;

				case RTCC_DATE_TIME_MENU:
					fSelection = SetRTCCDateTimeMenu(UARTid, pgcInputBuffer[0]);
					break;

				case READ_INPUTS_MENU:						// may be single pass or multiple pass display
					fSelection = ReadInputsMenu(UARTid, pgcInputBuffer[0]);
					break;

				case TEST_MENU:
					fSelection = TestMenu(UARTid, pgcInputBuffer[0]);
					break;
		...
		case ST_MENU_WAIT_FOR_PARAMETER_AND_CR:				// ST_MENU_WAIT_FOR_CR:
			...
			switch(bMenuFSMSequenceCtr)
				...
				case 0:
					...
					++bMenuFSMSequenceCtr;						// force next substate on next FSM entry
					bInputBufferIndex = 0;						// initialize input buffer index (input may be more than one character)
					break;
				...
				case 1:
					...
					if ((AnySerialRxDataAvailable(UARTid) IS_TRUE) AND (bInputBufferIndex < INPUT_BUFFER_SIZE))
					{
						...
						if (pgcInputBuffer[bInputBufferIndex] IS ASCII_CR)
						{
							// input is complete
						...
						else if ((eParameterType IS PARAMETER_TYPE_DECIMAL) AND (isdigit(pgcInputBuffer[bInputBufferIndex]) IS ZERO))
						{
							...
							DisplayMessage(UARTid, "\tNot a valid DECIMAL entry", NO_WAIT_FOR_DISPLAY);	// start display (serial output) of line
						...
						else if ((eParameterType IS PARAMETER_TYPE_BCD) AND (isdigit(pgcInputBuffer[bInputBufferIndex]) IS ZERO))
						{
							// allow digits
							// abort substate; only used when entry is invalid (NOT a digit)
							// note that this results in TWO writes within a single pass through the FSM (??), which may not work correctly...
							DisplayMessage(UARTid, "\tNot a valid BCD entry", NO_WAIT_FOR_DISPLAY);	// start display (serial output) of line
						...
						else if ((eParameterType IS PARAMETER_TYPE_HEX) AND (isxdigit(pgcInputBuffer[bInputBufferIndex]) IS ZERO))
						{
							// allow hex digits
							// abort substate; only used when entry is invalid (NOT a hex digit)
							// note that this results in TWO writes within a single pass through the FSM (??), which may not work correctly...
						//	DisplayMessage(UARTid, "\t\x01B[31m Not a valid HEX entry\x01B[30m", NO_WAIT_FOR_DISPLAY);	// start display (serial output) of line
                            DisplayMessage(UARTid, "\tNot a valid HEX entry", NO_WAIT_FOR_DISPLAY);
							fSubStateStatus = SUBSTATE_DONE;					// we are done with the state
							DelayMs(35);
                            Reset();
                            break;
						}
						else if ((eParameterType IS PARAMETER_TYPE_FLOATING_PT) AND (isdigit(pgcInputBuffer[bInputBufferIndex]) IS ZERO) AND (pgcInputBuffer[bInputBufferIndex] IS_NOT ASCII_PERIOD) AND (pgcInputBuffer[bInputBufferIndex] IS_NOT ASCII_MINUS))
						{
							// allow digits, period, minus sign
							// abort substate; only used when entry is invalid (NOT a digit AND NOT '.' AND NOT '-')
							// note that this results in TWO writes within a single pass through the FSM (??), which may not work correctly...
							DisplayMessage(UARTid, "\tNot a valid floating point entry", NO_WAIT_FOR_DISPLAY);	// start display (serial output) of line
                            DelayMs(35);
							fSubStateStatus = SUBSTATE_DONE;					// we are done with the state
							Reset();
				...
				case 2:
					...
					{
						// update parameter
						bValueValid = UpdateParameter(eParameter, &pgcInputBuffer[0]);	// update selected parameter based on contents of pgcInputBuffer[]
						if (bValueValid IS FALSE)
						{
							DisplayMessage(UARTid, "\tNot a valid value", NO_WAIT_FOR_DISPLAY);		// start display (serial output) of line
						}
					}
		...
		case ST_MENU_DISPLAY_STREAMING_DATA:
			switch(bMenuFSMSequenceCtr)
				...
				case 0:
					...
					++bMenuFSMSequenceCtr;					// force next substate on next FSM entry
					if (DisplayStatus(UARTid) IS_FALSE)		// returns FALSE if nothing to display
						{
						fSubStateStatus = SUBSTATE_DONE;	// force exit from state; no need for 2nd substate
						}
					break;
			...
			switch(bMenuFSMSequenceCtr)
				...
				case 1:
					...
					if(AnyRxDataAvailable(UARTid) IS_TRUE)
						...
						pgcInputBuffer[0] = (char)ReadRxdData(UARTid);
						...
						if (eCurrentStatus IS RUNTIME_ERROR_STATUS)
							{
		...
		case ST_MENU_DISPLAY_RESULT:
			switch(bMenuFSMSequenceCtr)
				...
				case 0:
					...
					if (eResultLine IS RESULT_SINGLE_LINE)			// just one line to display
						{
						DisplayStr(UARTid);								// single line is already setup for display
						}
					// must be line 1 of 1 to n lines..
					else if (DisplayResult(UARTid) IS_FALSE)				// returns FALSE if nothing MORE to display
						{
						fSubStateStatus = SUBSTATE_DONE;			// force exit from state; no need for 2nd substate
						}
...
BOOL DisplayMenu(UART_MODULE UARTid)
{
	...
	if (strlen(fgpMenuText[fgwDisplayLineCtr]) IS (size_t)0)
		{
		// this is actually an error condition...
		eMenuLine = MENU_LAST_LINE;
	...
	DisplayStr(UARTid);
    DelayMs(100);// start display (serial output) of line
    if(bRetVal == FALSE)
    {
       ClearDisplayStr();
        AddDisplayStr("~");// this is (effectively) the last line to display
		DisplayStr(UARTid);
        DelayMs(100);
    }
...
//	BYTE ucPrevTrackingMode;
	float el=0;char temp[20];char szReturnStr3[DISPLAY_LINE_SIZE + 1];
	switch(cKeystroke)
	{
		case '1':				// Run UP
			#ifdef USE_ELEVATION
		...
		case '6':				// Set Current Orientation to Sun Position
			...
			DisplayMessage(SERIAL_MENU_UART,"Current Orientation = Sun Position\r\n", WAIT_FOR_DISPLAY);
		...
		case '9':				// Current Local Orientation
				...
				{
					DisplayMessage(SERIAL_MENU_UART,"Unable to Read Inclinometer\r\n", WAIT_FOR_DISPLAY);
				}
			#endif	//  USE_MMA8452Q_INCLINOMETER
		...
		case 'a':				// SPA Calculated Orientation
			/*{
				...
				strcpy(szfnDisplayStr, "SPA:   ");
				SPA_Backtrack_Format(szfnDisplayStr + strlen(szfnDisplayStr), &SPAOrientation);
				DisplayMessage(SERIAL_MENU_UART, szfnDisplayStr, WAIT_FOR_DISPLAY);

				// display orientation adjusted for Backtracking
				IGNORE_RETURN_VALUE AdjustForBacktracking(&SPAOrientation);
				strcpy(szfnDisplayStr, "Backtrk");
				Orientation_Format(szfnDisplayStr + strlen(szfnDisplayStr), &SPAOrientation);	// format for display
				DisplayMessage(SERIAL_MENU_UART, szfnDisplayStr, WAIT_FOR_DISPLAY);
				...
				// display adjusted SetPoint orientation
			...
			}*/
                    {
                            if (ReadRTCCDateTime(ptrDateTime) IS_NOT TRUE)
                            {
                                  //  RuntimeError(RX_MSG_ERROR_RTCC_READ_ERROR);
                                    ...
                                    DisplayMessage(SERIAL_MENU_UART, szReturnStr3, WAIT_FOR_DISPLAY);
                                    return FALSE;
                            }
                            else
                            {
...
int MotionSettingsMenu(UART_MODULE UARTid, char cKeystroke)
{
...
int OpenLoopMoveMenu(UART_MODULE UARTid, char cKeystroke)
{
...
int ClosedLoopMoveMenu(UART_MODULE UARTid, char cKeystroke)
{
	...
	AddDisplayStrAndNewLine(szfnDisplayStr);
	DisplayMessage(UARTid, szfnDisplayStr, WAIT_FOR_DISPLAY);
	ClearDisplayStr();

	// if the keystroke is 's', it can be processed to stop the current operation (some can be very long!)
	if ((cKeystroke IS_NOT 's') AND (cKeystroke IS_NOT 'r') AND (cKeystroke IS_NOT 'x'))
		{
	...
	switch(cKeystroke)
		...
		case '1':				// set move distace
			...
			AddDisplayStr("!");
            DisplayStr(UARTid);
            DelayMs(100);// start display (serial output) of line												// start display (serial output) of line
			nRetVal = PARAMETER_SELECTED;
...
//				pgfMoveDistanceDegrees[MOTOR_AZIMUTH] = -fgfCLMoveDistance;
		...
		case 's':				// stop
			// make sure there is an active motor to stop
			if (eClosedLoopActiveMotor IS_NOT MOTOR_NONE)
				{
...
int SystemParametersMenu(UART_MODULE UARTid, char cKeystroke)
{
...
int LocationParametersMenu(UART_MODULE UARTid, char cKeystroke)
{
	LOCAL ARRAY char szfnDisplayStr[DISPLAY_LINE_SIZE + 1];
	int nRetVal = COMMAND_SELECTED;
	int nStatus = 0;
...
int AzimuthParametersMenu(UART_MODULE UARTid, char cKeystroke)
{
//	LOCAL ARRAY char szfnDisplayStr[DISPLAY_LINE_SIZE + 1];
	int nRetVal = COMMAND_SELECTED;
	int nStatus = 0;
...
int ElevationParametersMenu(UART_MODULE UARTid, char cKeystroke)
{
	//LOCAL ARRAY char szfnDisplayStr[DISPLAY_LINE_SIZE + 1];
	int nRetVal = COMMAND_SELECTED;
	int nStatus = 0;
...
int BacktrackingParametersMenu(UART_MODULE UARTid, char cKeystroke)
{
...
int SetRTCCDateTimeMenu(UART_MODULE UARTid, char cKeystroke)
{
	LOCAL ARRAY char szfnDisplayStr[DISPLAY_LINE_SIZE + 1];
	int nRetVal = COMMAND_SELECTED;
	UINT8 cI2CData;
	...
	switch(cKeystroke)
	{
		case '1':
			...
			if (ReadRTCCRegister(DS3232_REG_SECONDS, &cI2CData)	IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
             //   AddDisplayStr("!");
           //     AddDisplayStr("@");
		...
		case '2':
			...
			if (ReadRTCCRegister(DS3232_REG_MINUTES, &cI2CData)	IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
		...
		case '3':
			...
			if (ReadRTCCRegister(DS3232_REG_HOURS, &cI2CData) IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
		...
		case '4':
			...
			if (ReadRTCCRegister(DS3232_REG_DAY, &cI2CData)	IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
		...
		case '5':
			...
			if (ReadRTCCRegister(DS3232_REG_DATE, &cI2CData) IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
		...
		case '6':
			...
			if (ReadRTCCRegister(DS3232_REG_MONTH, &cI2CData) IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
		...
		case '7':
			...
			if (ReadRTCCRegister(DS3232_REG_YEAR, &cI2CData)	IS TRUE)
			{
				BCDBytetoASCIIstr(cI2CData, szfnDisplayStr);
				AddDisplayStr(szfnDisplayStr);
				AddDisplayStr("  _");
...
int ReadInputsMenu(UART_MODULE UARTid, char cKeystroke)
{
...
int TestMenu(UART_MODULE UARTid, char cKeystroke)
{
	int nRetVal = COMMAND_SELECTED;
	char cDivideByZero;
...
//	WORD wADCValue;
	...
	switch(eResultType)
		...
		case RESULT_STATUS_LAST_AZ_MOVEMENT:
			{
			if (eResultType IS RESULT_STATUS_LAST_AZ_MOVEMENT)
			{
				eMotor = MOTOR_AZIMUTH;
			}
			else if (eResultType IS RESULT_STATUS_LAST_EL_MOVEMENT)
			{
			...
			switch(fgwDisplayLineCtr)
				...
				case 12:
					...
					AddDisplayStr(szfnDisplayStr);
					AddDisplayStr("  Decel Min: ");
					WORDtoASCIIstr(pgMotionStats[eMotor].bPWM_DecelDutyCycleMin, BYTE_WIDTH, szfnDisplayStr);
					AddDisplayStr(szfnDisplayStr);
					++fgwDisplayLineCtr;														// bump display line counter to next line
					break;
		...
		case RESULT_READ_REGISTERS_MTN_SENSOR_SETTINGS:
			...
			switch(fgwDisplayLineCtr)
				...
				case 1:
					...
					INT32UtoHexASCIIstr(IC1CON, szfnDisplayStr);
					AddDisplayStr(szfnDisplayStr);
					++fgwDisplayLineCtr;														// bump display line counter to next line
					break;
				...
				case 2:
					...
					WORDtoHexASCIIstr(IC1BUF, szfnDisplayStr);
					AddDisplayStr(szfnDisplayStr);
					++fgwDisplayLineCtr;														// bump display line counter to next line
					break;
		...
		// AddDisplayStr("\tFBORPOR: BOR AND POR Device Configuration Register: ");
...
// TBD: timers 4, 5
		...
		case RESULT_READ_REGISTERS_RTCC:
			...
			switch(fgwDisplayLineCtr)
				...
				case 3:
					AddDisplayStr("Register 0x02 Hours, AM/PM:\t");
					if (ReadRTCCRegister(DS3232_REG_HOURS, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
				...
				case 8:
					...
					if (ReadRTCCRegister(DS3232_REG_ALARM1, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 9:
					...
					if (ReadRTCCRegister(DS3232_REG_CR, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 10:
					...
					if (ReadRTCCRegister(DS3232_REG_SR, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
		...
		case RESULT_READ_REGISTERS_INCLINOMETER:
			...
			switch(fgwDisplayLineCtr)
				...
				case 1:
					...
					if (ReadInclinometerRegister(STATUS_00_REG, &cI2CData)	IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 2:
					...
					if (ReadInclinometerRegister(OUT_X_MSB_REG, &cI2CData)	IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 3:
					...
					if (ReadInclinometerRegister(OUT_X_LSB_REG, &cI2CData)	IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 4:
					...
					if (ReadInclinometerRegister(OUT_Y_MSB_REG, &cI2CData)	IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 5:
					...
					if (ReadInclinometerRegister(OUT_Y_LSB_REG, &cI2CData)	IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 6:
					...
					if (ReadInclinometerRegister(OUT_Z_MSB_REG, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 7:
					...
					if (ReadInclinometerRegister(OUT_Z_LSB_REG, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 8:
					...
					if (ReadInclinometerRegister(SYSMOD_REG, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 9:
					...
					if (ReadInclinometerRegister(INT_SOURCE_REG, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 10:
					...
					if (ReadInclinometerRegister(WHO_AM_I_REG, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 11:
					...
					if (ReadInclinometerRegister(CTRL_REG1, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 12:
					...
					if (ReadInclinometerRegister(CTRL_REG2, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
				...
				case 13:
					...
					if (ReadInclinometerRegister(CTRL_REG3, &cI2CData) IS TRUE)
					{
						BYTEtoHexASCIIstr(cI2CData, szfnDisplayStr);					// this actually reads the hardware
						AddDisplayStr(szfnDisplayStr);
					}
			...
			switch(fgwDisplayLineCtr)
				...
				case 1:
					...
					if (ReadInclinometerRegister(OUT_X_MSB_REG, &cI2CData)	IS TRUE)
					{
						...
						if ((cI2CData & OUT_SIGN_MASK) IS OUT_SIGN_MASK)	// check for negative number
						{
	...
	if (eResultLine IS_NOT RESULT_LAST_LINE)
	...
	switch(eCurrentStatus)
		{
		 ...
		 // ==>> only applies to USE_PWM_DUTY_CYCLE
			case MOTION_ERROR_STATUS:
				...
				if ((fgwMotionErrorDisplayIndex <= pgwMotionProfileSpeedErrorIndex) AND (fgwMotionErrorDisplayIndex < SPEED_ERROR_TABLE_LEN))
					{
		...
		case POLAR_AXIS_MOVE_STATUS:
				...
				// note that we do not actually remove anything from the buffer; we just bump a display index that is initialized to 0
				if ((fgbPolarAxisMoveDisplayIndex < POLAR_AXIS_MOVE_TBL_LEN) AND (pgPolarAxisMoveTbl[fgbPolarAxisMoveDisplayIndex].fAzimuthAngleDegrees IS_NOT 0.0))
					{
					eStreamLine = STREAM_LINE;				// mark display of {some} line of data
...
//					strcat(szfnDisplayStr, (const char *)ftoa2(pgPolarAxisMoveTbl[fgbPolarAxisMoveDisplayIndex]., (int *)&nStatus));
		...
		case RUNTIME_ERROR_STATUS:
			...
			// display the contents of the runtime error buffer
			// note that we do not actually remove anything from the buffer; we just bump a display index that is initialized to 0
			if ((fgwErrorDisplayIndex < ERROR_BUFFER_SIZE) AND (pgwRuntimeErrorBuffer[fgwErrorDisplayIndex] IS_NOT ZERO))
				{
				eStreamLine = STREAM_LINE;				// mark display of {some} line of data
...
void CurrentMechanicalOrientation_Format(char *ptrOutputStr)
{
	...
	strcpy(ptrOutputStr, "\t");		// start string buffer
		...
		strcat(ptrOutputStr, (const char *)ftoa2(Orientation.fAzimuth, (int *)&nStatus));
	#endif
	#ifdef USE_ELEVATION
		strcat(ptrOutputStr, "  El: ");
		strcat(ptrOutputStr, (const char *)ftoa2(Orientation.fElevation, (int *)&nStatus));
	#endif
...
void CurrentLocalOrientation_Format(char *ptrOutputStr)
{
	...
	CurrentLocalOrientation_Read(&Orientation);
	strcpy(ptrOutputStr, "\t");		// start string buffer
	#ifdef USE_AZIMUTH
		strcat(ptrOutputStr, "Az: ");
		strcat(ptrOutputStr, (const char *)ftoa2(Orientation.fAzimuth, (int *)&nStatus));
                strcat(ptrOutputStr, " Ticks: ");
