    ...
    switch(eSunPosState)
		...
		case ST_PANEL_POS_INIT:
			...
			if ((ptrOrientation->fAzimuth < (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
				OR (ptrOrientation->fAzimuth > (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward - ptrRAM_SystemParameters->fAZ_Offset))
				#ifdef USE_ELEVATION
...
*/
			// check for SPA Orientation within Soft Limits
			if ((ptrOrientation->fAzimuth > (ptrRAM_SystemParameters->fSingle_SoftLimit_Reverse - ptrRAM_SystemParameters->fAZ_Offset))
				AND (ptrOrientation->fAzimuth < (ptrRAM_SystemParameters->fSingle_SoftLimit_Forward  - ptrRAM_SystemParameters->fAZ_Offset))
				#ifdef USE_ELEVATION
					AND (ptrOrientation->fElevation > (ptrRAM_SystemParameters->fEL_SoftLimit_Reverse - ptrRAM_SystemParameters->fEL_Offset))
					AND (ptrOrientation->fElevation < (ptrRAM_SystemParameters->fEL_SoftLimit_Forward - ptrRAM_SystemParameters->fEL_Offset))
				#endif	// USE_ELEVATION
