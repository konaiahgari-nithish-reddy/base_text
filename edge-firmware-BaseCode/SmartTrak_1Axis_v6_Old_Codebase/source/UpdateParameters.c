...
//	UINT16 nParameterValue;
	...
	switch(eParameterToUpdate)
	{
		...
		case PARAMETER_LATITUDE:
			...
			// bounds check
			if ((fParameterValue >= MIN_LATITUDE) AND (fParameterValue <= MAX_LATITUDE))
				{
		...
		case PARAMETER_LONGITUDE:
			...
			// bounds check
			if ((fParameterValue >= MIN_LONGITUDE) AND (fParameterValue <= MAX_LONGITUDE))
				{
		...
		case PARAMETER_ALTITUDE:
			...
			// bounds check
			if ((fParameterValue >= MIN_ALTITUDE) AND (fParameterValue <= MAX_ALTITUDE))
				{
		...
		case PARAMETER_REFRACTION:
			...
			// bounds check
			if ((fParameterValue >= MIN_REFRACTION) AND (fParameterValue <= MAX_REFRACTION))
				{
		...
		case PARAMETER_TIMEZONE:
			...
			// bounds check
			if ((fParameterValue >= MIN_TIMEZONE) AND (fParameterValue <= MAX_TIMEZONE))
				{
		...
		case PARAMETER_TRACKING_MODE:
			...
			// bounds check
			if ((bParameterValue >= MODE_MANUAL) AND (bParameterValue <= MODE_WIND_STOW))
				{
		...
		case PARAMETER_MOVE_DISTANCE:							// for closed loop menu
			...
			// bounds check
			if ((fParameterValue >= 0.0) AND (fParameterValue <= 90.0))
				{
		...
		case PARAMETER_X_COORD:									// for Move Sequence menu, Operation
			...
			// bounds check
			if ((fParameterValue >= -90.0) AND (fParameterValue <= 90.0))
				{
		...
		// **************************************
			...
			case PARAMETER_AZ_OFFSET:
				...
				// bounds check
				if ((fParameterValue >= AZ_SOFT_LIMIT_DEGREES_REVERSE) AND (fParameterValue <= AZ_SOFT_LIMIT_DEGREES_FORWARD))
					{
			...
			case PARAMETER_AZ_DEAD_BAND:
				...
				// bounds check
				if ((fParameterValue >= 0.0) AND (fParameterValue <= 2.0))
					{
			...
			case PARAMETER_AZ_NIGHT_STOW_THRESHOLD:
				...
				// bounds check
				if ((fParameterValue >= -10.0) AND (fParameterValue <= 10.0))
					{
		...
		case PARAMETER_EL_OFFSET:
			...
			// bounds check
			if ((fParameterValue >= EL_SOFT_LIMIT_DEGREES_REVERSE) AND (fParameterValue <= EL_SOFT_LIMIT_DEGREES_FORWARD))
				{
		...
		// **************************************
			...
			case PARAMETER_BACKTRACKING_ENABLED:
				...
				// bounds check
				if ((bParameterValue IS 0) OR (bParameterValue IS 1))
					{
			...
			case PARAMETER_SUN_SHADOW_START_ANGLE:
				...
				// bounds check
				if ((fParameterValue >= 0) AND (fParameterValue <= 90))
					{
		...
		case PARAMETER_SUN_SHADOW_START_HEIGHT:
				...
				// bounds check
				if ((fParameterValue >= 0.0) AND (fParameterValue <= AZ_SOFT_LIMIT_DEGREES_FORWARD))
					{
			...
			// bounds check
			if ((fParameterValue >= AZ_SOFT_LIMIT_DEGREES_REVERSE) AND (fParameterValue <= 0))
				{
			...
			// bounds check
			if ((fParameterValue >= 0) AND (fParameterValue <= AZ_SOFT_LIMIT_DEGREES_FORWARD))
				{
		...
		case PARAMETER_RTCC_SECONDS:
			...
			if ((bParameterValue >= DS3232_MIN_SECONDS) AND (bParameterValue <= DS3232_MAX_SECONDS))
				{
				...
				if (WriteRTCCRegister(DS3232_REG_SECONDS, bParameterValue) IS_FALSE)
					{
					DisplayMessage(SERIAL_MENU_UART, "I2C Write Failure", WAIT_FOR_DISPLAY);
					}
				}
			else
				{
		...
		case PARAMETER_RTCC_MINUTES:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_MINUTES) AND (bParameterValue <= DS3232_MAX_MINUTES))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
		...
		case PARAMETER_RTCC_HOURS:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_HOURS_24) AND (bParameterValue <= DS3232_MAX_HOURS_24))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
		...
		case PARAMETER_RTCC_DAY:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_DAY) AND (bParameterValue <= DS3232_MAX_DAY))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
		...
		case PARAMETER_RTCC_DATE:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_DATE) AND (bParameterValue <= DS3232_MAX_DATE))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
		...
		case PARAMETER_RTCC_MONTH:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_MONTH) AND (bParameterValue <= DS3232_MAX_MONTH))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
		...
		case PARAMETER_RTCC_YEAR:
			bParameterValue = (UINT8)atoi(ptrValueBuffer);		// convert input string to UINT8
			if ((bParameterValue >= DS3232_MIN_YEAR) AND (bParameterValue <= DS3232_MAX_YEAR))
				{
				bParameterValue = BYTEtoBCD(bParameterValue);		// convert UINT8 to BCD
